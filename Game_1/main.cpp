...
int main(int argc,char *argv[])
{
	...
	unsigned char* img = stbi_load("../res/textures/house_grayscale.jpg", &width, &height, &numComponents, 0);
	if (img == NULL)
	{
	...
	scn->AddTexture("../res/textures/house_grayscale.jpg", false);
	scn->SetShapeTex(0, 0);
	...
	byte* haltonedImg = halftoning_algo(img, width, height);
	s = stbi_write_png("../res/textures/haltone.png", width*2, height*2, numComponents, haltonedImg, width*2);
	scn->AddTexture("../res/textures/haltone.png", false);
	scn->SetShapeTex(0, 2);
	...
	write_img_to_file("img5.txt", haltonedImg, width * height* 4, width*2, true);
	write_img_to_file("img6.txt", fsImg, width * height, width, false);
	stbi_image_free(img);
	stbi_image_free(edgeImg);
	stbi_image_free(haltonedImg);
